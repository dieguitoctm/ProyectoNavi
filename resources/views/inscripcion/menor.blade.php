...
@section('content')
    ...
    <div class="container-fluid p-0">
        <div class="form-wrapper">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-center gap-3 mb-4">
                ...
                <a href="{{ route('inscripcion.despedida', $usuario->id) }}" class="btn btn-success">No, finalizar
                    inscripción</a>
            </div>
            ...
            <form id="form-menor" method="POST" action="{{ route('menor.guardar', $usuario->id) }}"
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="fecha_nacimiento" id="fecha_nacimiento"
                        min="{{ \Carbon\Carbon::now()->subYears(5)->addDay()->format('Y-m-d') }}"
                        max="{{ \Carbon\Carbon::now()->format('Y-m-d') }}" value="{{ old('fecha_nacimiento') }}"
                        class="form-control" required autocomplete="off" placeholder="Seleccione fecha">
                    <div class="invalid-feedback">El menor debe tener menos de 5 años (máx. 4 años 11 meses 29 días) y no
                        puede ser una fecha futura.</div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Validación fecha (solo máximo: menos de 5 años y no futura)
            form.addEventListener('submit', function (event) {
                const minDate = new Date(fechaNacimiento.min); // fecha más antigua permitida (hace 5 años + 1 día)
                const maxDate = new Date(fechaNacimiento.max); // hoy
                const selectedDate = new Date(fechaNacimiento.value);

                if (isNaN(selectedDate.getTime()) || selectedDate < minDate || selectedDate > maxDate) {
                    fechaNacimiento.setCustomValidity('El menor debe tener menos de 5 años (máx. 4 años 11 meses 29 días) y no puede ser una fecha futura.');
                } else {
